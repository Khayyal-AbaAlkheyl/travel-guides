...
(function (global) {
  ...
  function img(url, alt, ratio, focal, fallback) {
    ...
    const fb = fallback && fallback !== src ? esc(fallback) : '';
    const err = fb
      ? ` onerror="if(this.dataset.fb){this.src=this.dataset.fb;this.dataset.fb=''}" data-fb="${fb}"`
      : '';
    return `<figure class="media" style="aspect-ratio:${esc(r)}">
      <img src="${esc(src)}" alt="${esc(alt || '')}" loading="lazy" style="object-position:${esc(pos)}"${err}>
    </figure>`;
  }

  function photoUrl(photos, key, altKey) {
    if (!photos) return '';
    return photos[key] || photos[altKey] || photos.hero || photos.signature || '';
  }
